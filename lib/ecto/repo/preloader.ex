defmodule Ecto.Repo.Preloader do
    ...
    {embed_structs, counts} =
      Enum.flat_map_reduce(structs, [], fn
        %{^field => embeds}, counts when is_list(embeds) ->
          {embeds, [Enum.count(embeds) | counts]}
  ...
  defp load_embeds(
         ...
         field,
         [struct | structs],
         [embed_struct | embed_structs],
         [1 | counts],
         acc
       ),
       do:
         load_embeds(:one, field, structs, embed_structs, counts, [
           Map.put(struct, field, embed_struct) | acc
         ])
      ...
      struct, {fetch_ids, loaded_ids, loaded_structs} ->
        ...
        cond do
          ...
          card == :many and loaded? ->
            {fetch_ids, [{id, Enum.count(value)} | loaded_ids],
             Enum.concat(value, loaded_structs)}
  ...
  defp load_assoc({:assoc, assoc, ids}, struct) do
    ...
    as = Map.get(assoc, :as, List)

    loaded =
      case ids do
        %{^key => value} -> if(as != List, do: apply(as, :new, [value]), else: value)
  ...
  defp load_through({:through, assoc, throughs}, struct) do
    ...
    as = Map.get(assoc, :as, List)
    IO.put("loads-through")
    Map.put(struct, field, maybe_first(loaded, cardinality, as))
  end

  defp maybe_first(list, :one, _), do: List.first(list)
  defp maybe_first(list, _, List), do: list
  defp maybe_first(list, _, as), do: apply(as, :new, [list])
  ...
  defp merge_preloads(_preload, {info, _, nil, left}, {info, take, query, right}),
    do: {info, take, query, Enum.concat(left, right)}
